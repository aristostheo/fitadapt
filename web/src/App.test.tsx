...
import type { PersonalizedPlanSnapshot, ProfileIntelligenceResponse } from './types'
...
function response(stage: 'baseline' | 'calibrating' | 'early_personalized' | 'personalized' = 'personalized', progression = false): ProfileIntelligenceResponse {
  const plan: PersonalizedPlanSnapshot = { as_of_date: '2026-01-14', lifecycle_stage: stage, lifecycle_requirements: [], calorie_basis: stage === 'personalized' ? 'personalized' : 'baseline', baseline_calorie_target_kcal_per_day: 2300, selected_calorie_target_kcal_per_day: 2450, previous_selected_calorie_target_kcal_per_day: 2400, change_from_previous_snapshot_kcal_per_day: 50, adaptive_tdee_kcal_per_day: 2450, adaptive_median_absolute_deviation_kcal_per_day: 6, eligible_adaptive_estimate_count: 8, recommendation_status: 'increase_calories', recommendation_reasons: ['adjustment_recommended'], raw_recommendation_adjustment_kcal_per_day: 160, limited_recommendation_adjustment_kcal_per_day: 150, macro_plan: { calorie_target_kcal_per_day: 2450, calorie_source: 'personalized', strategy: 'higher_protein', body_weight_kg: 80, protein_g_per_kg: 2, fat_percentage: .25, protein_g_per_day: 160, fat_g_per_day: 68, carbohydrate_g_per_day: 298, protein_kcal_per_day: 640, fat_kcal_per_day: 613, carbohydrate_kcal_per_day: 1192, macro_policy_version: 'preference_macros_v1', assumptions: [] }, planning_policy_version: 'personalized_planning_v1', baseline_energy_formula_version: 'mifflin', baseline_activity_policy_version: 'activity', baseline_energy_equivalent_policy_version: 'energy', baseline_macro_policy_version: 'baseline', lifecycle_policy_version: 'lifecycle', trend_policy_version: 'trends', adaptive_policy_version: 'adaptive', recommendation_policy_version: 'recommendation', personalized_macro_policy_version: 'preference_macros_v1', assumptions: [] }
...
describe('unified profile intelligence workflow', () => {
  ...
  it('renders current personalized macro-plan values, MAD variability copy, and chart empty state', async () => {
    mockApi(); render(<App />); fireEvent.click(screen.getByRole('button', { name: 'Analyze my profile' })); await screen.findByText('CURRENT PROPOSED PLAN'); expect(screen.getByText('160 g/day')).toBeInTheDocument(); expect(screen.getByText(/MAD describes observed variability, not statistical confidence/)).toBeInTheDocument(); expect(screen.getByText('No recorded steps values yet. Add entries to see this chart.')).toBeInTheDocument()
  })
  ...
  it('renders baseline plans, a personalized fallback, and no raw recommendation codes as primary copy', async () => {
    ...
    expect(screen.getByText('baseline basis')).toBeInTheDocument(); expect(screen.getByText(/Legacy V0\.1 baseline macros are intentionally secondary/)).toBeInTheDocument()
