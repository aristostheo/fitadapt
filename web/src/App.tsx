...
import {
  ...
  type ImportFormat,
  type ImportPreview,
} from "./import/observation-import";
...
import { TrainingContext as TrainingContextEditor } from "./components/TrainingContext";
...
const initialTrainingContext: TrainingContext = {
...
export default function App() {
  ...
  const [dietaryProfile, setDietaryProfile] =
    useState<NutritionPreferenceProfile>(initialDietaryProfile);
  ...
  const [results, setResults] = useState<ProfileIntelligenceResponse | null>(
    null,
  );
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [lastRequest, setLastRequest] =
    useState<ProfileIntelligenceRequest | null>(null);
  const [showImport, setShowImport] = useState(false);
  const [importFormat, setImportFormat] = useState<ImportFormat>("json");
  const [importText, setImportText] = useState("");
  const [preview, setPreview] = useState<ImportPreview | null>(null);
  const [importMode, setImportMode] = useState<"merge" | "replace_all">(
    "merge",
  );
  const [conflictPolicy, setConflictPolicy] = useState<
    "reject_conflicts" | "keep_existing" | "replace_existing"
  >("reject_conflicts");
  ...
  const request = (): ProfileIntelligenceRequest => ({
    ...
    nutrition_preferences:
      preferences.macro_strategy === "custom"
        ? preferences
        : { macro_strategy: preferences.macro_strategy },
    dietary_preference_profile: dietaryProfile,
    training_context: trainingContext,
    include_plan_progression: includeProgression,
  });
  ...
  const updatePreference = (strategy: MacroStrategy) => {
    setPreferences(
      strategy === "custom"
        ? {
            macro_strategy: strategy,
            custom_protein_g_per_kg: 1.8,
            custom_fat_percentage: 0.25,
          }
        : { macro_strategy: strategy },
    );
    clearStale();
  };
  const updateCustomPreference = (updates: Partial<NutritionPreferences>) => {
    setPreferences((current) => ({ ...current, ...updates }));
    clearStale();
  };
  const updateDietaryProfile = (next: NutritionPreferenceProfile) => {
    setDietaryProfile(next);
    clearStale();
  };
  const updateDraft = (key: keyof Observation, value: string) => {
    const next = { ...draft } as Record<
      string,
  ...
  const saveObservation = () => {
    ...
    if (!draft.observed_on || !measurement) {
      setError(
        "An observation date and at least one measurement are required.",
      );
      return;
    }
    if (
    ...
    setObservations((current) =>
      sorted([
        ...current.filter((item) => item.observed_on !== editing),
        draft,
      ]),
    );
  ...
  const startPreview = (text: string, format = importFormat) => {
    setImportText(text);
    ...
    setPreview(
      previewObservationImport(
        text,
        format,
        observations.map((item) => item.observed_on),
      ),
    );
    setImportSummary("");
  };
  const importFile = async (file: File | undefined) => {
    ...
    const format: ImportFormat = file.name.toLowerCase().endsWith(".csv")
      ? "csv"
      : "json";
    startPreview(await file.text(), format);
  };
  const confirmImport = () => {
    if (!preview) return;
    if (preview.errors.length > 0 && !allowValidOnly) {
      setError(
        "Fix import errors or explicitly choose to import valid rows only.",
      );
      return;
    }
    if (
      ...
      setError("Resolve date conflicts before merging.");
      return;
    }
    const incoming = preview.validRows;
    let next: Observation[];
    let skipped = 0;
    let replaced = 0;
    if (importMode === "replace_all") next = incoming;
    else if (conflictPolicy === "keep_existing") {
      next = [
        ...observations,
        ...incoming.filter(
          (item) => !preview.conflictingDates.includes(item.observed_on),
        ),
      ];
      skipped = preview.conflictingDates.length;
    } else if (conflictPolicy === "replace_existing") {
      next = [
        ...observations.filter(
          (item) => !preview.conflictingDates.includes(item.observed_on),
        ),
      ...
      replaced = preview.conflictingDates.length;
    } else next = [...observations, ...incoming];
    setObservations(sorted(next));
    setImportSummary(
      `Imported ${incoming.length - skipped} rows; skipped ${skipped}; replaced ${replaced}; invalid ${preview.errors.length}.`,
    );
    setPreview(null);
    setImportText("");
    setDemoLoaded(false);
  ...
  const loadDemo = () => {
    setObservations(createFictionalSample(profile));
    setShowAll(false);
    setDemoLoaded(true);
    clearStale();
  };
  const analyze = async (nextRequest = request()) => {
    ...
    if (
      nextRequest.nutrition_preferences.macro_strategy === "custom" &&
      ((nextRequest.nutrition_preferences.custom_protein_g_per_kg ?? 0) < 1.2 ||
        (nextRequest.nutrition_preferences.custom_protein_g_per_kg ?? 0) >
          2.4 ||
  ...
  const stateFor = (step: JourneyStep): JourneyState => {
    ...
    const currentIndex = JOURNEY.findIndex((item) => item.id === currentStep);
    const index = JOURNEY.findIndex((item) => item.id === step);
    if (step === "nutrition" && dietaryErrors.length > 0) return "incomplete";
    if (index < currentIndex) return "completed";
    if (step === "history")
      return observations.length > 0 ? "ready" : "incomplete";
    if (step === "plan" || step === "progress")
      return results ? "ready" : "not started";
    return "ready";
  };
    ...
    >
      ...
      {currentStep === "nutrition" && (
        <NutritionStep
          ...
          dietaryErrors={dietaryErrors}
          onStrategy={updatePreference}
          onCustom={updateCustomPreference}
      ...
      {currentStep === "history" && (
        <HistoryStep
          ...
          onEdit={(item) => {
            setDraft(item);
            setEditing(item.observed_on);
          }}
          onDelete={(observedOn) => {
            setObservations((current) =>
              current.filter((value) => value.observed_on !== observedOn),
            );
            setDemoLoaded(false);
            clearStale();
          }}
      ...
      {currentStep === "plan" && (
        <PlanStep
          ...
          onIncludeProgression={(value) => {
            setIncludeProgression(value);
            clearStale();
          }}
          onAnalyze={() => void analyze()}
          onRetry={() => lastRequest && void analyze(lastRequest)}
          onBack={() => setCurrentStep("history")}
          onProgress={() => setCurrentStep("progress")}
        />
      )}
      {currentStep === "progress" && (
        <ProgressStep result={results} onBack={() => setCurrentStep("plan")} />
      )}
    </AppShell>
  );
}
